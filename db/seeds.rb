...
5.times do
  ...
  rand(3..10).times do
    ...
    car = Car.new(brand: brand, category: category, model: model, year: Faker::Vehicle.year, location: Faker::Address.city, description: "#{image.description.nil? ? "" : image.description}: The car license of is #{Faker::Vehicle.license_plate}. It is a #{Faker::Vehicle.fuel_type} car, with #{Faker::Vehicle.transmission} transmition. Its original color from factory is #{Faker::Vehicle.color}.")
    car.owner = user
